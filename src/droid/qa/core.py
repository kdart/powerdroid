...
class Test(object):
  ...
  # TODO(dart) replace this with some new meta-data holder.
#   @classmethod
#   def SetTestOptions(cls, dbcase):
#     """Copy database data to Test class options."""
#     InsertOptions(cls)
#     opts = cls.OPTIONS
#     opts._set = True
#     opts.db_id = dbcase.id
#     opts.name = dbcase.name
#     opts.interactive = dbcase.interactive
#     opts.automated = dbcase.automated
#     opts.valid = dbcase.valid
#     opts.priority = dbcase.priority
#     opts.cycle = dbcase.cycle
#     opts.status = dbcase.status
#     opts.prerequisites = filter(None,
#          map(lambda tcp: PreReq(tcp.testcase.testimplementation,
#                             tcp.testcase.id, tcp.args, tcp.kwargs),
#                    iter(dbcase.prerequisites.all())))
#     opts.bugid = dbcase.bugid # Also signals "known failure" report response.
